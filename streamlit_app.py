...
.stApp { background: var(--bg-void); color: var(--text); }
...
.console-title {
    ...
    text-shadow: 0 0 12px rgba(53,242,160,0.35);
    margin: 0;
}
...
with c4:
    model_choice = st.radio("MODEL", ["Random Forest", "1D-CNN"], label_visibility="visible")
...
if run:
    ...
    with scope_col:
        st.markdown('<div class="panel"><div class="panel-label">CONSTELLATION SCOPE — live sweep</div>', unsafe_allow_html=True)
        scope_ph = st.empty()
        ...
        decode_ph.markdown(
            ...
            <div class="panel">
                ...
                <div class="mono" style="color:{status_color}; font-size:0.8rem; margin:6px 0 14px;">
                    ● {status_text} &nbsp;|&nbsp; input: {true_mod} @ {snr_db} dB
                </div>
                {led_rows}
            </div>
            ''',
